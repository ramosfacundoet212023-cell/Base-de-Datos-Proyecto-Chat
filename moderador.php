...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['crear_aula'])) {
    ...
    // Obtenemos tu ID de moderador desde la sesión
    $id_moderador = $_SESSION['user_id']; 
    ...
    if (!empty($nombre_aula) && !empty($clave_aula)) {
        ...
        // Agregamos ID_AGBD a la consulta SQL
        $stmt_aula = $conn->prepare("INSERT INTO Aula (Nombre_Aula, Clave, ID_AGBD) VALUES (?, ?, ?)");
        
        // Cambiamos "ss" a "ssi" (string, string, integer) para incluir tu ID
        $stmt_aula->bind_param("ssi", $nombre_aula, $hash_clave, $id_moderador);
